(()=>{
 ...
 function selectNearby(index=0,automatic=false){const water=nearbyRows[index];if(!water)return;window.selectedWater={...water};if(position){position={...position,waterbody_id:water.id||null,water_name:water.name,water_type:water.water_type,state_code:water.state_code};if(byId('mWater'))window.FishWizzGuard?.setGuardedValue?.(byId('mWater'),water.name||'','water');updatePositionCard();document.dispatchEvent(new CustomEvent('atlas:water-selected',{detail:{water,automatic,position:window.atlasFishingLocation}}))}drawBestMarker(water);if(!automatic&&typeof loadWater==='function')loadWater(water)}
