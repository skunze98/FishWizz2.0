(()=>{
 ...
 // P1/P2 (release-blocking, 2026-08-28, NO-GO QA -- "Build Mission here
 // shows 'Spot ready' but the Water field is empty" / "Map simultaneously
 // says the water is unmatched AND identified"): d.selected_water is only
 // ever a COORDINATE reverse lookup -- this RPC's own nearest-indexed-water
 // match against the exact pin. It can legitimately come back null even
 // when the user already searched for and picked an exact, named water
 // earlier in the very same flow (window.selectedWater, set at selection
 // time and never cleared). This panel's header and its "Build Mission
 // here" button used to trust ONLY the coordinate lookup -- so a real,
 // user-verified water identity was silently discarded the instant the
 // reverse lookup didn't independently confirm it (header: "No indexed
 // water selected"), while window.selectedWater still held the name
 // unchanged the whole time, so anything else on screen reading it kept
 // showing the water as identified -- exactly the reported contradiction.
 // The coordinate match is the more precise result when it exists (a real
 // distance/state/type computed from the pin); the user's own explicit
 // search selection is the fallback of record, never silently dropped.
 function render(d){const box=ensurePanel(),rpcWater=d.selected_water,fallbackWater=!rpcWater&&window.selectedWater?window.selectedWater:null,w=rpcWater||fallbackWater,q=d.data_quality||{},missing=Array.isArray(q.missing_layers)?q.missing_layers:[],catches=d.recent_catches||[],spots=d.saved_spots||[];if(last)last.__lastData=d;box.innerHTML=`<span class="eyebrow">Exact-position intelligence</span><h2>${esc(w?.name||'No indexed water selected')}</h2>${rpcWater?`<p><b>${esc(rpcWater.water_type)}</b> · ${esc(rpcWater.state_code)} · ${esc(fmt(rpcWater.distance_miles))} from pin</p>`:fallbackWater?'<p class="muted tiny">Matched from your search selection -- not yet independently confirmed at this exact pin.</p>':''}<p><b>Data quality:</b> ${esc(q.quality_grade||'Not graded')}${q.quality_score!=null?` · ${esc(q.quality_score)}/100`:''}</p>${depthFact(d._depth)}${layerControls(d)}${missing.length?`<p class="muted tiny"><b>Still missing:</b> ${esc(missing.join(', '))}</p>`:''}${spotsHtml(spots)}${catches.length?`<details><summary>Your recent history on this water (${catches.length})</summary>${catches.slice(0,8).map(c=>`<p class="tiny"><b>${esc(c.species||'Catch')}</b> · ${esc(c.lure_bait||'unknown lure')}${c.color?` · ${esc(c.color)}`:''}${c.combo_name?` · ${esc(c.combo_name)}`:''}<br><span class="muted">${esc(c.spot||'Unnamed spot')}${c.learned?` — ${esc(c.learned)}`:''}</span></p>`).join('')}</details>`:''}<div class="row"><button id="contextMission" class="btn gold">Build Mission here</button><button id="refreshContext" class="btn ghost">Refresh</button></div><p class="muted tiny">Atlas maps verified coordinates when available. Older catches without GPS stay linked to the water/spot name rather than receiving invented map pins.</p>`;bindControls();box.querySelectorAll('[data-spot-edit]').forEach(b=>b.onclick=()=>editSpot(b.dataset.spotEdit));box.querySelectorAll('[data-spot-delete]').forEach(b=>b.onclick=()=>deleteSpot(b.dataset.spotDelete));$('contextMission').onclick=()=>{if(w){window.selectedWater={...(window.selectedWater||{}),...w};if($('mWater'))window.FishWizzGuard?.setGuardedValue?.($('mWater'),w.name,'water');if($('mWaterType')&&w.water_type)$('mWaterType').value=/river|stream/i.test(w.water_type)?(/stream/i.test(w.water_type)?'Stream':'River'):'Lake'}showPage('mission');setTimeout(()=>$('liveWeather')?.click(),150)};$('refreshContext').onclick=()=>last&&load(last)}
 ...
 globalThis.__fishwizzTest=Object.assign(globalThis.__fishwizzTest||{},{mapContext:{render,load}});
})();