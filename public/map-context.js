(()=>{
 ...
 function layerControls(d){
  ...
  if(d._depth?.available)rows.push(`<label><input type="checkbox" data-layer="depth" checked> Depth contours (${d._depth.contour_count})</label>`);
 ...
 function draw(d){clear();
  ...
  if(d._depth?.available)(d._depth.contours||[]).forEach(c=>contourLine(c.paths,c.depth_ft,d._depth.max_depth_ft));
 }
 function depthFact(depth){
  ...
  return `<p><b>Depth chart:</b> ${esc(depth.max_depth_ft)} ft max (DNR survey, ${esc(depth.contour_count)} contour lines at ${esc(depth.contour_interval_ft)}-ft intervals). Estimated from a lake survey -- always confirm with sonar on the water.</p>`;
 }
