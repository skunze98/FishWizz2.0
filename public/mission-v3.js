...
[['mTarget','target'],['mSeason','season'],['mClarity','clarity'],['mWind','wind'],['mLight','light'],['mAccess','access'],['mCover','cover'],['mCurrent','current'],['mDepth','depth'],['mSky','sky'],['mPrecip','precipitation'],['mPressure','pressure_trend'],['mWaterTemp','water_temp'],['mLevel','water_level_trend'],['mActivity','fish_activity'],['mWaterType','water_type']].forEach(([id,key])=>setSelect(id,c[key]));if(Number.isFinite(Number(c.latitude))&&Number.isFinite(Number(c.longitude))){window.atlasFishingLocation={lat:Number(c.latitude),lon:Number(c.longitude),method:'saved_mission',accuracy:c.position_accuracy_m||null,waterbody_id:c.waterbody_id||null};document.dispatchEvent(new CustomEvent('atlas:fishing-position',{detail:window.atlasFishingLocation}))}window.atlasLiveWeather=null;showPage('mission');stat('Last Mission loaded. Refreshing it with current conditions…','ok');await buildPlan()}catch(e){stat(e.message||'Could not repeat the last Mission.','err')}}
 ...
 // Found while adding evidence labeling above (not that bug itself, but hit
 // it live while testing): p was read unsafely (p.combo, no ?.) even
 // though the line right before it already guarded with p?.inventory_fit
 // -- get_mission_plan_v3 can legitimately return backup/finesse as null
 // for some target/condition combinations, which used to crash this whole
 // render into a raw "Cannot read properties of null" error instead of
 // just omitting that one card. If a null card is ever passed, returning
 // '' now (never mind if none of the three calls in buildPlan() intend
 ...
 async function buildPlan(){
  ...
  try{
   ...
   // "Start here:" -- every get_mission_plan_v3 start_zone value is itself a
   // full sentence beginning with "Start" ("Start on a current break...",
   // "Start with shade...", etc, confirmed in supabase/schema/public.sql),
   ...
   // (and every primary/backup/finesse card below) come from
   // get_mission_plan_v3 -- a deterministic rules engine, not a live or
   // official-record lookup (see that function's own SQL body: fixed
   // if/else technique guidance). estimatedLine says so explicitly, right
   // next to the plan itself, instead of leaving that text carrying the
   // same unlabeled authority as the real LIVE weather reading below it.
   ...
   const estimatedLine=`<p class="muted tiny">${chip('estimated','ESTIMATED — general technique guidance')} This plan is Atlas's general fishing-technique guidance for these conditions, not a location-verified or live-agency reading.</p>`;
   // "For manually entered water without exact position, explicitly label
   // plan as generic/non-spatial": mission-why.js already said this inside
