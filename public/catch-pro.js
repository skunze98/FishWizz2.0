(()=>{
 ...
 // P2-13 ("field-level errors ... announced accessibly"). .fw-field-error is
 // also used by gear-catalog.js/manual-gear-pro.js, but that stylesheet is
 // only injected on the Arsenal/Tackle page groups -- never loaded here, so
 ...
 // angler with an empty Species field saw "Sign in to save catches" and,
 ...
 async function enhancedSave(){if(busy)return;const water=$('cWater')?.value.trim(),species=$('cSpecies')?.value.trim();if(!water||!species){stat('Add the water and species before saving.','err');markRequired('cWater',!water);markRequired('cSpecies',!species);return}markRequired('cWater',false);markRequired('cSpecies',false);
  ...
  // remained while Spot, Species, size and release reset"), an undocumented
  // and inconsistent special case. fillFromMission() below is the one
  // mechanism that repopulates ANY field from the current Mission/spot/combo
  // context -- the same rule for every field, not two different rules for
  // different fields. It only fills what's empty, so it can't silently
