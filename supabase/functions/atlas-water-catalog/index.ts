...
const geometryGeojson=feature.geometry&&feature.geometry.type&&feature.geometry.coordinates?JSON.stringify(feature.geometry):null;
const {data:id,error}=await admin.rpc("upsert_catalog_waterbody",{p_source_system:source.key,p_source_id:sourceId,p_source_label:source.label,p_name:name,p_state_code:state,p_water_type:source.type,p_lon:lon,p_lat:lat,p_official_url:q.toString(),p_source_updated_at:null,p_geometry_geojson:geometryGeojson});if(!error)waters.push({id,name,state_code:state,water_type:source.type,source_label:source.label,latitude:lat,longitude:lon,official:true});}}catch(e){failures.push({source:source.key,error:String(e)})}}));
