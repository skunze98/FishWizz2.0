...
| Full-accuracy mapping project, Phase 3c (water clarity) | **Researched and verified live, not yet executed.** MN's Citizen Lake Monitoring Program publishes a real, queryable per-station Secchi transparency summary (`services.pca.state.mn.us/api/v1/cmp/loc-data`) -- confirmed live against the program's own human-facing report page for a real station (`webapp.pca.state.mn.us/vmp/stations/21-0057-00-204/print`) before committing to it, which resolved a real unit ambiguity: the API's field docs (on a *different* endpoint) say Secchi values are in meters for lakes, but the cross-check proves `loc-data`'s lake-station values are actually in **feet** (19.75 ft on both, exactly) -- the schema/migration comments record this so nobody re-guesses it later. `waterbodies.clarity_secchi_ft`/`clarity_year`/`clarity_sample_count`/`clarity_source` columns already exist (same migration as Phase 3b) and are ready to receive it. **Wisconsin's equivalent (DNR satellite-derived Secchi for ~8,000 lakes) ships only as raster imagery, not a queryable per-lake value** -- filling it would mean sampling a raster per lake, a materially different and bigger task than everything else in this project, so it's left unfilled rather than approximated. **What's left before running it:** `loc-data` requires a `county` or `huc8` filter (no statewide query), so the import needs a real enumerated list of MN's 87 counties or HUC8 codes to iterate -- not yet written, to avoid guessing at county name spelling/coverage without the same verification rigor as everything else in this project. River/stream stations also use a different, unverified unit convention and are out of scope regardless (clarity is a lakes-only column, matching Phase 2). |
