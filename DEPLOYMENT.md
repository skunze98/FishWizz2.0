...
| Full-accuracy mapping project, Phase 1 (access points) | **Deployed live, 2026-08-26.** User asked for the app to cover every public shore fishing site, boat landing, and canoe/kayak launch in MN and WI, not just what a live tap happens to discover. Real counts confirmed before starting: 3,025 MN sites (`struc_water_access_sites`) + 3,277 WI sites (142 shore fishing + 3,135 boat access, `PR_Boat_Access_Shore_Fishing_WTM_Ext`) = 6,302. `scripts/import-access-points.mjs` bulk-fetches every one (paginated, `orderByFields` added after the first run surfaced 27 duplicate rows from unstable ArcGIS pagination ordering -- also de-duped defensively regardless of cause) and upserts into `public_access_points` on the same `(source_system, source_id)` key `atlas-water-access` already uses, so it's idempotent and re-runnable to pick up new sites later. **6,275 unique rows confirmed live** in `doddeferfxzgdmzadibq`. 577 were immediately linkable to a `waterbody_id` via a best-effort name match against the water catalog as it exists today (small -- 1,325 rows -- see the next row) -- those are already visible today on the water-profile page and in the map's access-point layers, which read `public_access_points` directly. The remaining ~5,700 were imported unlinked (`waterbody_id: null`) pending Phase 2 -- see the linking-pass update in that row below, which raised the total to 5,631/6,275 (90%) linked. **Not yet done:** rivers/streams (deliberately out of scope for this pass -- see Phase 2's row), then a depth/temp/clarity backfill pass. Weather is unaffected -- already live and coordinate-based, not tied to the catalog. |
| Full-accuracy mapping project, Phase 2 (water-body catalog + access-point linking + a real production bug found along the way) | **Deployed live, 2026-08-26.** Bulk-imported every named, real lake/pond basin in MN and WI with actual shoreline geometry (not just a centroid) -- scoped to named/significant waters, per the user's own choice, not literally every one of the ~112,000 raw GIS features (mostly tiny unnamed pond/wetland fragments). MN: `water_mn_public_waters`, filtered `pwi_class='P'` (a real lake/pond basin, not the "W" wetland classification) and named (`pw_basin_name<>'Unnamed'`) -- 8,203 raw features, verified live before writing the import. WI: the corrected `DW_Map_Dynamic/EN_SurfaceWater_WTM_Ext_Dynamic_L16` layer 5, `IN_STATE_CODE=1` and named -- 10,306 raw features, also verified live. `scripts/import-waterbodies-lakes.mjs` (first run surfaced two real bugs, both fixed before the successful run -- see its own header comment: WI's service silently caps a filtered query below the requested page size without signaling it, which cut the first WI fetch off at 1,000/10,306; and some real lakes are split across multiple polygon features sharing one DOW/WBIC, which raced on a database constraint when dispatched concurrently -- now grouped and merged into one MultiPolygon per real id instead) upserts via the same `upsert_catalog_waterbody` RPC the live per-tap path already uses. **Second run: 8,202 MN + 6,812 WI = 15,014 lakes, 0 failures**, real PostGIS-derived centroid and shoreline geometry for 15,038 of the now-16,325-row catalog (up from 22 rows with real geometry, out of 1,325 total, before this pass).

Also fixed while scoping this: `atlas-nearby-waters` and `atlas-water-catalog` were both requesting a `dnr_hydro_id` field for MN's lake source that **does not exist** on that layer (verified live), silently falling back to a name-based id -- harmless for a unique name, but MN has many real, distinct lakes sharing a common name across counties ("Rice Lake", "Long Lake", etc.), so a second one tapped later would have upserted onto the first one's catalog row instead of getting its own. Switched both to `dowlknum`, confirmed real and non-zero on every row this import touches.

**A second, more serious production bug found immediately after, while running the access-point linking pass:** `nearby_water_catalog` -- the Postgres-side fallback `atlas-nearby-waters` calls whenever a live DNR fetch fails or times out, so a real user path, not just this session's scripts -- started failing every call with `57014 canceling statement due to statement timeout`. Root cause: the function filters/orders by `coalesce(geometry::geography, centroid)`, a computed expression, but the existing indexes are on the raw `geometry` and `centroid` columns separately -- Postgres cannot use either for that expression, so every call fell back to a sequential scan computing real distance against every row's shoreline. Invisible at ~1,325 sparse rows; a hard timeout at 16,325 rows with 15,038 real polygons. Fixed with a real GiST expression index (`supabase/migrations/20260826150000_waterbodies_geog_expr_index.sql`) matching the function's actual expression, applied directly via `supabase db query --linked` (Management API, no DB password needed -- local/remote migration history has an unrelated pre-existing mismatch from before this session that would need its own separate reconciliation, so this one migration was applied standalone rather than blocked on that). **Verified**: the identical call that previously hard-timed-out now returns in ~1.2s with a correct `on_water`, distance-0 match.

`scripts/link-access-points.mjs` then reconciled Phase 1's still-unlinked access points against the new catalog, using the same `nearby_water_catalog` RPC (real shoreline distance, not a name guess) the app itself trusts. **5,054 of 5,698 newly linked** (the timeout above blocked the first attempt entirely -- 0 linked -- until the index fix). Combined with Phase 1's initial 577 name-matched: **5,631 of 6,275 access points (90%) now linked to a real water**, visible today on the water-profile page and the map's access-point layers. 644 remain unlinked -- genuinely more than 0.5 mile from any cataloged water, not a bug, plausibly a coordinate-precision or true-data-quality issue in the source DNR records for those specific sites, not investigated further this pass.

**Not done this pass, deliberately:** rivers/streams -- MN's named-stream count alone is in the thousands, segmented into ~130,000 individual line pieces with no single natural "one row" geometry the way a lake basin has, and live per-tap stream matching already works correctly now that the WI cross-border bug is fixed, so the comprehensiveness gap that motivated the lake import doesn't apply the same way to streams. A smaller, separate pass if wanted. Also not done at the time: depth/temp/clarity backfill for the newly-imported lakes (see Phase 3 below) and reconciling the pre-existing local/remote migration history mismatch the index fix had to work around rather than fix. |
| Full-accuracy mapping project, Phase 3a (real-time gauges) | **Deployed live, 2026-08-26.** `stream_gauges` held exactly 1 row -- `atlas-live-water` already knows how to refresh live values for gauges that exist in the table, it just never had a way to discover new ones. `scripts/import-gauges.mjs` pulls USGS's own Site Service (`waterservices.usgs.gov/nwis/site`, `seriesCatalogOutput=true` for real per-site parameter lists, not just existence) for MN+WI, scoped to real-time (`hasDataTypeCd=iv`) stream/lake sites reporting discharge, gage height, or water temperature -- the three parameters the app already knows how to read. **171 MN + 449 WI = 620 gauges**, upserted on the table's real `(agency, site_id)` unique constraint. `scripts/link-gauges.mjs` then linked them to a real cataloged water via the same `nearby_water_catalog` RPC access points use -- **308/620 linked**. Most gauges sit on rivers/streams (deliberately not bulk-cataloged in Phase 2), so a lower hit rate than lakes' 90% is expected, not a bug. |
| Full-accuracy mapping project, Phase 3b (persisted depth) | **Deployed live, 2026-08-26.** Added nullable `max_depth_ft`/`depth_kind`/`depth_source`/`depth_updated_at` columns to `waterbodies` (migration `20260826160000_waterbodies_depth_clarity_columns.sql`, applied via `supabase db query --linked` same as the index fix -- the pre-existing migration-history mismatch still isn't reconciled). `scripts/backfill-depth.mjs` is efficient by construction: Phase 2 already gave every lake a real `dowlknum`(MN)/`WBIC`(WI) as its `source_id`, so this queries both DNR services directly `WHERE dowlknum IN (...)`/`WHERE WBIC IN (...)` in batches (30 dowlknums/batch for MN -- small enough to stay clear of the bathymetry service's 2,000-feature cap even for a many-contour lake; 150 WBICs/batch for WI, one row per lake so no real risk there) instead of one request per lake. **1,840/8,202 MN lakes have a real DNR bathymetric survey on file (~22%) + 4,831/7,303 WI lakes have a reported max depth (~66%) = 6,671 of 15,014 lakes (44%) now carry a real, persisted depth**, zero `exceededTransferLimit` truncations across every batch. Verified: the three real, distinct "Elk" lakes across different MN counties correctly show three different real depths (90/15/25 ft, not colliding), and Lake Mendota shows its real known 83 ft. |
| Full-accuracy mapping project, Phase 3c (water clarity) | **Attempted 2026-08-26; blocked by a real CAPTCHA wall, not fixable from this session.** MN's Citizen Lake Monitoring Program publishes a real, queryable per-station Secchi transparency summary (`services.pca.state.mn.us/api/v1/cmp/loc-data`) -- confirmed live against the program's own human-facing report page for a real station before committing to it, which resolved a real unit ambiguity (the API's field docs, on a *different* endpoint, say meters; the cross-check proves `loc-data`'s lake-station values are actually **feet** -- 19.75 ft on both, exactly). `loc-data` needs a `county` or `huc8` filter (no statewide query), so `scripts/backfill-clarity.mjs` iterates all 80 of MN's real HUC8 major watersheds -- pulled live from MN DNR's own watershed layer (`geos_dnr_watersheds/FeatureServer/6`), not typed from memory. **First run: the very first request succeeded** -- 42 real lake stations, 26 matched to a cataloged lake via `nearby_water_catalog` and written to `waterbodies.clarity_secchi_ft` -- **then every request after that failed.** Diagnosed as request-pacing sensitivity; added a real 600ms delay and backed-off retries. **Second run revealed the actual cause: every request now returns a Radware Bot Manager CAPTCHA page instead of JSON.** That's a genuine bot-detection wall, not a transient error pacing or retries can fix, and it will not be solved or routed around in any form -- no amount of slower pacing, retrying, or otherwise, since evading a triggered bot-detection system is exactly the thing not to do regardless of the goal. The 26 lakes already filled in are genuine data from before the block triggered, left as-is, not backed out. **A real path forward is non-programmatic**: MPCA offers a bulk data request/export process for exactly this kind of use case -- that's a request for the user to make directly, not something to script around the block. Wisconsin's clarity equivalent (DNR satellite-derived Secchi for ~8,000 lakes) remains unfilled regardless, for the separate reason that it ships only as raster imagery, not a queryable per-lake value. |
| Full-accuracy mapping project, Phase 4 (rivers/streams) 🔑 | **Deployed live, 2026-08-26.** The piece phase 2 deliberately deferred (133,523 raw MN segments state-wide, no single natural "one row" geometry the way a lake basin has). Same overall approach as phase 2 -- group real segments into one row per real river, upsert via `upsert_catalog_waterbody` -- but the grouping key differs by state, verified live before choosing it: **WI's `RIVER_SYS_WBIC` is a real, stable per-river-*system* id** (confirmed live: all 19 segments of the real Bad River share one WBIC, the same safe pattern WI lakes already use). **MN has no equivalent** -- `kittle_nbr` is per-segment/reach, not per-river (confirmed live: the Mississippi River's 83 MN segments each carry a different `kittle_nbr`) -- so MN is grouped by `kittle_name` itself, a real but lower-stakes tradeoff than lakes' old bug (streams carry no depth/temp/clarity data to misattribute if two differently-located same-named creeks ever merge). Scoped to real, natural, non-intermittent named waterways: MN via `strm_type_desc IN ('Centerline (River)','Stream (Perennial)')` -- verified live via a full type breakdown before choosing it, cutting 133,523 raw segments to 27,632 by excluding intermittent streams, administrative connector lines, agricultural ditches, and buried infrastructure (storm sewers, drain tile) that happens to carry a name in this layer; WI via `IN_STATE_CODE=1` and named (27,351 raw segments, no further filtering needed since WBIC grouping is already correct). **1,680 MN + 3,122 WI = 4,802 distinct rivers/streams**, 1 failure (a statement timeout on one large merged geometry, not chased further -- 4,802/4,803 is a solid success rate). `waterbodies` is now 21,007 rows total, 19,835 with real geometry.

**Bug found and fixed the same day, before it could compound:** the first run correctly excluded WI's literal `'Unnamed'` placeholder but not MN's equivalent (`'Unnamed Creek'`, `'Unnamed Ditch'`) -- MN has no per-river id to fall back on the way WI's WBIC provides, so grouping by name merged every genuinely different, unrelated "Unnamed Creek" segment state-wide into one nonsensical combined shape. Caught immediately: only 2 MN rows were actually affected (most literally-unnamed MN segments were already correctly cataloged one at a time by the live per-tap path, with a real per-segment `dnr_hydro_id`, from before this script ever ran) -- confirmed via source-id pattern, not guessed. Fixed in the script (excludes `/^unnamed\b/i` names from MN's grouping now, matching WI's existing exclusion) so a future re-run won't recreate it. **Still needed 🔑, not done by this session:** the 2 already-bad rows (verified zero dependent rows in any user-data table) are in `scripts/cleanup-bad-unnamed-mn-streams.sql`, handed to the user the same way the phase-2 cleanup was -- a destructive action against live data, not something to push through automatically. Separately confirmed as *not* a bug: 197 WI rows still display as "Unnamed" despite the filter -- each has a real, distinct WBIC (no merging), just a genuinely blank name field in WI's own data for that reach. |
| QA Tracker P0-P3 (identity leak guard, atomic Mission state, river/lake matching, gear CRUD, Profile nav, gear-tab dedup, real session state machine, onboarding dedup, guest drafts, catch-form reset, nav hit-targets, validation order, ARIA tab/filter semantics, mobile nav containing-block bug, mojibake + duplicated-copy + Walleye wording data fixes) | **Deployed live, 2026-08-27**, working through `FishWizz_QA_Tracker.docx` in strict priority order, one batch at a time, each with its own regression suite (`scripts/test-p0-fixes.mjs` through `test-p3-fixes.mjs`, 26+22+18+5 = 71 checks) plus `test-mentor-explanations.mjs` re-run clean after every batch. Full command-by-command root cause/fix/test writeup lives in this session's own transcript, not restated here -- this row is the deploy record. Highlights: a real CSS containing-block bug (`header{backdrop-filter}` made it a containing block for `.nav`'s `position:fixed` mobile bottom-bar mode, confirmed live via `getBoundingClientRect` -- the entire mobile nav was rendering clipped near the top of the page, not docked at the bottom, at every viewport the tracker named as broken); two duplicate, competing implementations found and removed (`inventory-add.js` vs `manual-gear-pro.js` for gear creation, `launch.js` vs `onboarding.js` for onboarding); a real "Start here: Start on a current break..." duplicated-sentence bug fixed in both `mission-v3.js` and the `ask-atlas` edge function (redeployed); 8 mojibake ("â€“" for a real en dash) rows repaired directly in the live database across rods/reels/lures/combos, and `get_mission_plan_v2`'s Walleye/X-Rap `primary_watch` text corrected from live-bait wording ("a tick") to real jerkbait feedback -- both applied via `supabase db query --linked -f` since neither function has a tracked migration (both predate the migration history). **Frontend confirmed live** at `fishwizz-e7d.pages.dev` (spot-checked `location-state.js`/`guest-draft.js` existing, `map.js` containing `resolveNamedWater`, `styles.css` no longer containing header's `backdrop-filter`, `landing-app-theme.css` containing the `.nav button[hidden]` fix -- all present in the deployed files after the CDN's brief post-deploy propagation window passed). `app.fishwizz.com` is unaffected either way -- confirmed via the Pages API that its domain record is still `status: pending, "CNAME record not set"`, unchanged from the pre-existing gap documented above; the user does not have DNS access to fishwizz.com to create it. **Not deployed this pass**: P4+ (the tracker has no P4; all commands through P3 are now live). **Not verified**: a live authenticated click-through -- same standing constraint all session, Turnstile correctly blocks scripted sign-in and it is not being routed around. |
